...
//!
//! TODO(jason): Extract `env_var`, `Var`, and `VarFunction` into a shared crate
//! (e.g. `dbt-jinja-functions`) so dbt-profile and dbt-jinja-utils use the
//! exact same code without duplication. For now, the implementations below are
//! copy-pasted from `dbt-jinja-utils/src/functions/base.rs`.
...
/// Trait for var() function behavior, shared between Var implementations.
///
/// Copy-pasted from `dbt-jinja-utils/src/functions/base.rs::VarFunction`.
