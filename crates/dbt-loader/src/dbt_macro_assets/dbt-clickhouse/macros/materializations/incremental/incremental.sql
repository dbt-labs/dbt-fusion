  ...
  {% else %}
    ...
    {#-
      ...
      In Fusion those methods don't exist; instead we:
        ...
        2. Call adapter.get_incremental_strategy_macro() which validates the strategy against
           valid_incremental_strategies() for ClickHouse via the Rust enum — but discard the
           returned macro object since ClickHouse uses inline dispatch rather than the generic
           get_incremental_{strategy}_sql dispatch pattern.
    ...
    {% do adapter.get_incremental_strategy_macro(context, incremental_strategy) %}
