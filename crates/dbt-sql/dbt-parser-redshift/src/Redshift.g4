...
/* 
 * This is dbt's Antlr grammar file for Redshift SQL.
 *
 ...
 {logical='Arc<LogicalPlan>,Vec<FunctionRegistration>,Option<TargetName>,*Properties,StatementType,*Option<TableType>,*Vec<CteInfo>,*UpstreamEntities';}
    ...
    | <logical='Arc<LogicalPlan>,Option<TargetName>,Option<TableType>'>
      CREATE EXTERNAL TABLE
        ...
        redshiftCreateExternalTableClauses                             #redshiftCreateExternalTable
    | <logical='Arc<LogicalPlan>,Option<TargetName>,Option<TableType>,UpstreamEntities'>
      CREATE EXTERNAL TABLE
        dest=qualifiedName
        redshiftCreateExternalTableAsClauses
        AS (query | LPAREN query RPAREN)                                     #redshiftCreateExternalTableAs
    | <logical='Arc<LogicalPlan>,Option<TargetName>,Option<TableType>,UpstreamEntities'>
      CREATE
        ...
        AS (query | LPAREN query RPAREN)
        (WITH NO SCHEMA BINDING)?                                      #redshiftCreateTableAsSelect
    ...
    | <logical='Arc<LogicalPlan>,Option<TargetName>,Option<TableType>'>
      CREATE
        ...
        (LPAREN tableElements tail=COMMA? RPAREN)?
         (BACKUP (YES | NO))?
         redshiftTableAttributes                                       #redshiftCreateTable
    | <logical='Arc<LogicalPlan>,Option<TargetName>'>
    ...
    | <logical='Arc<LogicalPlan>,Option<TargetName>,Option<TableType>,UpstreamEntities'>
      CREATE (OR REPLACE)? MATERIALIZED VIEW
        ...
        (COMMENT string)?
        (WITH properties)? AS query                                    #redshiftCreateMaterializedView
    | <logical='Arc<LogicalPlan>,Option<TargetName>,Option<TableType>,UpstreamEntities'>
      CREATE (OR REPLACE)? VIEW
        dest=qualifiedName
        columnAliases?
        (COMMENT comment=string)?
        (SECURITY (DEFINER | INVOKER))? AS (query | LPAREN query RPAREN)
        (WITH NO SCHEMA BINDING)?                                     #redshiftCreateView
    | <logical='Arc<LogicalPlan>,Option<TargetName>,UpstreamEntities'>
      SHOW COLUMNS FROM TABLE tableName=qualifiedName                       #showColumns
    | <logical='Option<TargetName>,Vec<FunctionRegistration>'>
      CREATE (OR REPLACE)? EXTERNAL? FUNCTION
      name=qualifiedName
      LPAREN ( namedParameter (COMMA namedParameter )* tail=COMMA? )? RPAREN
      RETURNS return_type=type_
      functionPropertySpec+                                            #createFunction
    ...
    */
    ...
    | ABORT (WORK|TRANSACTION)?                                        #abort
    | ALTER ~SEMI_COLON*                                                      #alter
    | ATTACH ~SEMI_COLON*                                                     #attach
    | BEGIN ~SEMI_COLON*                                                      #begin
    | CANCEL ~SEMI_COLON*                                                     #cancel
    | CLOSE ~SEMI_COLON*                                                      #close
    | COPY ~SEMI_COLON*                                                       #copy
    | SET ~SEMI_COLON*                                                        #set
    ...
    | <logical='Option<TargetName>,Arc<LogicalPlan>,UpstreamEntities'>
      EXPLAIN VERBOSE? statement                                              #explain
    ...
    | CREATE EXTERNAL SCHEMA ~SEMI_COLON*                                     #createExternalSchema // redshift only
    | CREATE GROUP ~SEMI_COLON*                                               #createGroup // redshift only
    | CREATE IDENTITY ~SEMI_COLON*                                            #createIdentity // redshift only
    | CREATE (OR REPLACE)? PROCEDURE ~(';'|'$$')* DOLLAR_QUOTED_STRING ~SEMI_COLON* #createProcedure // redshift only
    | CREATE USER ~SEMI_COLON*                                                #createUser // redshift only
    | CREATE DATABASE ~SEMI_COLON*                                            #createFoo // redshift only
    | CREATE DATASHARE ~SEMI_COLON*                                           #createFoo // redshift only
    | CREATE (OR REPLACE)? LIBRARY ~SEMI_COLON*                               #createFoo // redshift only
    | CREATE MASKING ~SEMI_COLON*                                             #createFoo // redshift only
    | CREATE MODEL ~SEMI_COLON*                                               #createFoo // redshift only
    | CREATE RLS ~SEMI_COLON*                                                 #createFoo // redshift only
    | DECLARE ~SEMI_COLON*                                                    #declare // redshift only
    | DETACH ~SEMI_COLON*                                                     #detach // redshift only
    | END ~SEMI_COLON*                                                        #end // redshift only
    | FETCH ~SEMI_COLON*                                                      #fetch // redshift only
    | LOCK ~SEMI_COLON*                                                       #lock // redshift only
    | UNLOAD ~SEMI_COLON*                                                     #unload // redshift only
    | VACUUM ~SEMI_COLON*                                                     #vacuum // redshift only
    ;
...
redshiftCreateExternalTableClauses options {logical='Properties';}
    :
    (locationSpec
    ...
    | tableProperties)*
    ;
redshiftCreateExternalTableAsClauses options {logical='Properties';}
    :
    (locationSpec
...
rowFormatedSpec // redshift only
    : ROW FORMAT (
          DELIMITED ((LINES|FIELDS) TERMINATED BY string)+
    )
    ;

rowFormatedAndSerdeSpec // redshift only
    : ROW FORMAT (
          DELIMITED ((LINES|FIELDS) TERMINATED BY string)+
        | SERDE string (WITH SERDEPROPERTIES LPAREN (string EQ string)(COMMA string EQ string)* tail=COMMA? RPAREN)?
    )
    ;

tableProperties // redshift only
    : TABLE PROPERTIES LPAREN (string EQ string)(COMMA string EQ string)* tail=COMMA?RPAREN
    ;

functionPropertySpec
    : LANGUAGE identifier                                          #functionLanguage
    | IMMUTABLE                                               #functionVolatility
    | STABLE                                      #functionVolatility
    | VOLATILE                                  #functionVolatility
    | IAM_ROLE ( DEFAULT | string)                              #functionIAMRole
    | LAMBDA string                                             #functionLambda
    | RETRY_TIMEOUT number                                    #functionRetryTimeout
    | MAX_BATCH_ROWS number                                  #functionMaxBatchRows
    | MAX_BATCH_SIZE number (KB|MB)                         #functionMaxBatchSize
    | SAGEMAKER string                                       #functionSagemaker
    | AS DOLLAR_QUOTED_STRING                          #functionBody
    ;
...
tableConstraint
    ...
    | PRIMARY KEY columnAliases?
        | FOREIGN KEY columnAliases? REFERENCES qualifiedName (LPAREN identifier RPAREN)
    )
    ;

columnDefinition options {logical='Field';}
    : fieldDefinition
    (columnAttributes | columnConstraints)* (COMMENT comment=string)?
    ;
fieldDefinition options {logical='Field';}
    : columnName columnSchemaWithMetadata
    | strictNonReserved columnSchemaWithMetadata
    ;
...
columnAttributes
    // Redshift wiki missed the CONSTRAINT
    : (
        (DEFAULT expression)
        | (IDENTITY (LPAREN number COMMA number tail+=COMMA? RPAREN)?)
        | (GENERATED BY DEFAULT AS IDENTITY (LPAREN number COMMA number tail+=COMMA? RPAREN)?)
        | (ENCODE identifier)
        | DISTKEY
        | SORTKEY
...
columnConstraints
    : (
        (NOT NULL | NULL)
        | (UNIQUE | PRIMARY KEY)
        | (REFERENCES qualifiedName LPAREN identifier RPAREN)
    )+
    ;

likeClause options {logical='Schema';}
    : LIKE qualifiedName
    (optionType=(INCLUDING | EXCLUDING)
        (
          PROPERTIES
        | DEFAULTS // redshift only
        )
    )?
    ;
...
redshiftTableAttributes
    : (
        ...
        | DISTKEY LPAREN expression RPAREN
        | ((COMPOUND | INTERLEAVED)? SORTKEY LPAREN (expression (COMMA expression)*)? tail+=COMMA? RPAREN | SORTKEY AUTO)
        | ENCODE AUTO
    )*
    ;
...
querySpecification options {logical='Arc<LogicalPlan>';}
    : SELECT
      ...
      ((START WITH expression)? CONNECT BY (PRIOR identifier EQ expression | expression GT PRIOR identifier) (START WITH expression)? )? // redshift only
      aggregationClause?
...
pivotedRelation options {logical='Arc<LogicalPlan>';}
    : pivotedRelationTarget pivotOperator*                        #pivotedRelationDefault
    | LPAREN pivotedRelation RPAREN                               #parenthesizedPivotedRelation
    ;
...
pivotInto options {logical='Expr,PivotColumnName';}
    : expression (AS columnName)?           #pivotIntoDefault
    ;
...
aliasedRelation options {logical='Arc<LogicalPlan>';}
    ...
    (AS? alias=identifier (columnAliases | AT indexAlias=identifier)?)?
    ;
columnAliases options {logical='Vec<Identifier>';}
    : identifier
    | LPAREN identifier (COMMA identifier )* tail=COMMA? RPAREN
    ;
...
relationPrimary options {logical='Arc<LogicalPlan>,TableReference,*Span';}
    ...
    | UNPIVOT expression AS value_alias=identifier (AT attribute_alias=identifier)?     #objectUnpivot
    ;
...
valueExpression options {logical='Expr,Argument,usize,f64,String,Identifier,Vec<Expr>,IntrinsicAlias';}
    ...
    | left=valueExpression operator=(BITWISE_AND | BITWISE_OR | BITWISE_XOR)
      right=valueExpression                                                             #arithmeticBinary
    | left=valueExpression operator=(BINARY_EXP | POSIX | POSIX_LIKE | POSIX_ILIKE | POSIX_NOT_LIKE | POSIX_NOT_ILIKE | POSIX_STAR)
                                          right=valueExpression                         #arithmeticBinary
    | left=valueExpression operator=BITWISE_SHIFT_RIGHT
      right=valueExpression                                                             #arithmeticBinary //Redshift only
    ;
...
primaryExpression options {logical='Expr,Argument,TableReference,usize,f64,String,Identifier,Vec<Expr>,IntrinsicAlias';}
    ...
    | (PG_CATALOG DOT)?(LISTAGG|LISTAGGDISTINCT) LPAREN setQuantifier? agg_exprs+=expression (COMMA agg_expr+=expression)? tail+=COMMA? RPAREN
    ...
    | EXTRACT LPAREN (string | identifier) FROM valueExpression RPAREN                          #redshiftExtract
    ...
    | primaryExpression AT timeZoneSpecifier                                              #atTimeZonePrimary
    ...
    | DOLLAR INTEGER_VALUE                                                                   #functionParameterColumnReference
    ...
    | JSON_ARRAY LPAREN
     ...
     RPAREN                                                                                  #jsonArray
    | VARIABLE                                                                            #variable
    | APPROXIMATE PERCENTILE_DISC LPAREN number RPAREN
      WITHIN GROUP LPAREN ORDER BY expression RPAREN                                            #approximateFunction // redshift only
    | APPROXIMATE COUNT LPAREN DISTINCT callArgument RPAREN                                     #approximateFunction // redshift only
    | CONVERT LPAREN type_ COMMA expression tail=COMMA? RPAREN                                    #convert // redshift only
...
functionName options {logical='Vec<Identifier>';}
    ...
    ;
namedParameter options {logical='Parameter';}
    :
    (IN | INOUT| OUT | VARIADIC)? identifier? type_
    ;
...
typeIdentifier options {logical='Identifier';}
    : identifier
    ;
type_ options {logical='DataType,GenericDataType';}
    : DOLLAR INTEGER_VALUE                                                             #functionSignatureGenericType // SDF yml only
    // Redshift does not support interval type as table column type, but we need it for view column type
    ...
    | base_=TIMESTAMP (LPAREN precision = typeParameter RPAREN)? WITH TIME ZONE           #dateTimeWithTzType
    | base_=TIME (LPAREN precision = typeParameter RPAREN)? (WITHOUT TIME ZONE)?          #dateTimeType
    | base_=TIME (LPAREN precision = typeParameter RPAREN)? WITH TIME ZONE                #dateTimeWithTzType
    | DOUBLE PRECISION                                                              #doublePrecisionType
    | CHARACTER VARYING (LPAREN INTEGER_VALUE RPAREN)?                                    #characterVarying
    | BINARY VARYING (LPAREN INTEGER_VALUE RPAREN)?                                       #binaryVarying
    | typeIdentifier
...
identifier options {logical='Identifier,*Option<UnquotedIdentifier>';}
    ...
    | LBRACKET IDENTIFIER RBRACKET  #unquotedIdentifier
    | quotedIdentifier       #quotedIdentifierDefault
    | LBRACKET quotedIdentifier RBRACKET  #quotedIdentifierDefault
    | nonReserved            #unquotedIdentifier
    | LBRACKET nonReserved RBRACKET  #unquotedIdentifier
...
nonReserved
    ...
    :
      ABORT | ABSENT | ADD | ADMIN | AFTER | ALL | ALTER | ANALYZE | AND | ANTI | ANY | APPROXIMATE | ARRAY | ASC | AT | ATTACH | AUTHORIZATION | AUTO
    | BACKUP | BEGIN | BERNOULLI | BETWEEN | BINARY | BINDING | BOTH | BY | BZIP2
    | CALL | CANCEL | CASCADE | CASE | CASE_INSENSITIVE | CASE_SENSITIVE | CAST | CATALOGS | CHARACTER | CLONE | CLOSE | CLUSTER | COLLATE | COLUMN | COLUMNS | COMMENT | COMMIT | COMMITTED | COMPOUND | COMPRESSION | CONDITIONAL | CONNECT | CONNECTION | CONSTRAINT | COPARTITION | COPY | COUNT | CREATE | CUBE | CURRENT | CURRENT_ROLE
    | DATA | DATABASE | DATASHARE | DATE | DAY | DAYS | DEALLOCATE | DECLARE | DEFAULT | DEFAULTS | DEFINE | DEFINER | DELETE | DELIMITED | DELIMITER | DENY | DESC | DESCRIBE | DESCRIPTOR | DETACH | DISTINCT | DISTKEY | DISTRIBUTED | DISTSTYLE | DOUBLE | DROP
    | ELSE | EMPTY | ENCODE | ENCODING | END | ERROR | ESCAPE | EVEN | EXCEPT | EXCLUDE | EXCLUDING | EXECUTE | EXISTS | EXPLAIN | EXTERNAL | EXTRACT
    | FALSE | FETCH | FIELDS | FILTER | FINAL | FIRST | FIRST_VALUE | FOLLOWING | FOR | FOREIGN | FORMAT | FROM | FUNCTION | FUNCTIONS
    | GENERATED | GRACE | GRANT | GRANTED | GRANTS | GRAPHVIZ | GROUP | GROUPING | GROUPS | GZIP
    | HAVING | HEADER | HOUR | HOURS
    | IAM_ROLE | IF | IGNORE | ILIKE | IMMUTABLE | IN | INCLUDE | INCLUDING | INITIAL | INOUT | INPUT | INPUTFORMAT | INSERT | INTERLEAVED | INTERSECT | INTERVAL | INTO | INVOKER | IO | IS | ISNULL | ISOLATION
    | JOIN | JSON | JSON_ARRAY | JSON_EXISTS | JSON_OBJECT | JSON_QUERY | JSON_VALUE
    | KB | KEEP | KEY | KEYS
    | LAG | LAMBDA | LANGUAGE | LAST | LAST_VALUE | LATERAL | LEADING | LEVEL | LIBRARY | LIKE | LIMIT | LINES | LISTAGG | LISTAGGDISTINCT | LOCAL | LOCATION | LOCK | LOGICAL
    | M | MAP | MASKING | MATCH | MATCHED | MATCHES | MATCH_RECOGNIZE | MATERIALIZED | MAX | MAX_BATCH_ROWS | MAX_BATCH_SIZE | MB | MEASURES | MERGE | MIN | MINUTE | MINUTES | MODEL | MONTH | MONTHS
    | NEXT | NFC | NFD | NFKC | NFKD | NO | NONE | NORMALIZE | NOTNULL | NULL | NULLS
    | OBJECT | OF | OFFSET | OMIT | ON | ONE | ONLY | OPTION | OPTIONS | OR | ORDER | ORDINALITY | OUT | OUTPUT | OUTPUTFORMAT | OVER | OVERFLOW
    | PARTITION | PARTITIONED | PARTITIONS | PASSING | PAST | PATH | PATTERN | PER | PERCENTILE_CONT | PERCENTILE_DISC | PERIOD | PERMUTE | PG_CATALOG | PIVOT | POSITION | PRECEDING | PRECISION | PREPARE | PRIMARY | PRIOR | PRIVILEGES | PROCEDURE | PROPERTIES | PRUNE
    | QUALIFY | QUOTES
    | RANGE | READ | RECURSIVE | REFERENCES | REFRESH | RENAME | REPEATABLE | REPLACE | RESET | RESPECT | RESTRICT | RETRY_TIMEOUT | RETURNING | RETURNS | REVOKE | RLS | ROLE | ROLES | ROLLBACK | ROLLUP | ROW | ROWS | RUNNING
    | S | SAGEMAKER | SCALAR | SCHEMA | SCHEMAS | SEC | SECOND | SECONDS | SECURITY | SEEK | SELECT | SEMI | SERDE | SERDEPROPERTIES | SERIALIZABLE | SESSION | SET | SETS | SHOW | SIMILAR | SOME | SORTKEY | SQL | STABLE | START | STATS | STRING_KW | STORED | STRUCT | SUBSET | SUBSTRING | SYSTEM_TIME
    | TABLE | TABLES | TABLESAMPLE | TEMP | TEMPORARY | TERMINATED | TEXT | THEN | TIES | TIME | TIMESTAMP | TO | TRAILING | TRANSACTION | TRIM | TRUE | TRUNCATE | TRY_CAST | TUPLE | TYPE
    | UESCAPE | UNBOUNDED | UNCOMMITTED | UNCONDITIONAL | UNION | UNIQUE | UNKNOWN | UNMATCHED | UNNEST | UNPIVOT | UNSIGNED | UPDATE | USE | USER | UTF16 | UTF32 | UTF8
    | VACUUM | VALIDATE | VALUE | VALUES | VARIADIC | VARYING | VERBOSE | VERSION | VIEW | VOLATILE
    | WEEK | WHEN | WINDOW | WITH | WITHOUT | WORK | WRAPPER | WRITE
    | XZ
    | YEAR | YEARS | YES
    | ZONE | ZSTD
...
LINES: 'LINES';
LISTAGG: 'LISTAGG';
LISTAGGDISTINCT: 'LISTAGGDISTINCT';
LOCAL: 'LOCAL';
LOCATION: 'LOCATION';
LOCK: 'LOCK';
LOGICAL: 'LOGICAL';
