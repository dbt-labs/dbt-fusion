...
fn build_parent_and_child_maps(nodes: &Nodes) -> (BTreeMap<String, Vec<String>>, BTreeMap<String, Vec<String>>) {
    ...
    for (id, _source) in &nodes.sources {
        // Sources usually don't depend on anything, but we ensure they exist in maps
        parent_map.entry(id.clone()).or_insert_with(Vec::new);
        child_map.entry(id.clone()).or_insert_with(Vec::new);
    }
    ...
    let all_parent_ids: Vec<String> = parent_map.values()
        .flat_map(|parents| parents.clone())
        .collect();
    
    for parent_id in all_parent_ids {
        parent_map.entry(parent_id.clone()).or_insert_with(Vec::new);
        child_map.entry(parent_id).or_insert_with(Vec::new);
    }
...
mod tests {
    use super::*;
    use crate::schemas::{Nodes,CommonAttributes};
    ...
    fn test_build_parent_and_child_maps_simple_dependency() {
        ...
        assert_eq!(parent_map.get("model.test.model_a").unwrap().len(), 0);
        assert_eq!(parent_map.get("model.test.model_b").unwrap(), &vec!["model.test.model_a".to_string()]);
        
        // Check child_map
        assert_eq!(child_map.len(), 1);
        assert_eq!(child_map.get("model.test.model_a").unwrap(), &vec!["model.test.model_b".to_string()]);
    }
    ...
    fn test_build_parent_and_child_maps_multiple_dependencies() {
        ...
        nodes.models.insert(
            ...
            create_test_model("model.test.model_c", vec![
                "model.test.model_a".to_string(),
                "model.test.model_b".to_string(),
            ]),
        );
        ...
        assert_eq!(
            ...
            &vec!["model.test.model_a".to_string(), "model.test.model_b".to_string()]
        );
        
        // Check child_map
        assert_eq!(child_map.len(), 2);
        assert_eq!(child_map.get("model.test.model_a").unwrap(), &vec!["model.test.model_c".to_string()]);
        assert_eq!(child_map.get("model.test.model_b").unwrap(), &vec!["model.test.model_c".to_string()]);
    }
    ...
    fn test_build_parent_and_child_maps_chain_dependency() {
        ...
        assert_eq!(parent_map.get("model.test.model_a").unwrap().len(), 0);
        assert_eq!(parent_map.get("model.test.model_b").unwrap(), &vec!["model.test.model_a".to_string()]);
        assert_eq!(parent_map.get("model.test.model_c").unwrap(), &vec!["model.test.model_b".to_string()]);
    ...
    fn test_build_parent_and_child_maps_with_source() {
        ...
        nodes.models.insert(
            "model.test.model_a".to_string(),
            create_test_model("model.test.model_a", vec!["source.test.my_source.table1".to_string()]),
        );
        ...
        // Check parent_map
        assert_eq!(parent_map.len(), 2);
        assert_eq!(parent_map.get("model.test.model_a").unwrap(), &vec!["source.test.my_source.table1".to_string()]);
        assert_eq!(parent_map.get("source.test.my_source.table1").unwrap().len(), 0);
        
        // Check child_map
        assert_eq!(child_map.len(), 1);
        assert_eq!(child_map.get("source.test.my_source.table1").unwrap(), &vec!["model.test.model_a".to_string()]);
    }
