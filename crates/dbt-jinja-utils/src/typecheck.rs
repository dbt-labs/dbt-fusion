...
#[allow(clippy::too_many_arguments)]
/// Extract macro/function call dependencies for a batch of files.
///
/// This mirrors [`typecheck`] plumbing (context + listener lifecycle), but runs the dependency
/// analyzer instead of full typechecking. Dependencies are emitted via
...
pub fn analyze_dependencies(
    ...
    let function_signatures = env.jinja_function_registry.clone();
    let mut jinja_env = env.env.clone();
    let macro_namespace_registry = env.env.get_macro_namespace_registry();
    let builtins = load_builtins_with_namespace(macro_namespace_registry)
        .map_err(|e| FsError::from_jinja_err(e, "Failed to load built-ins"))?;
    ...
    jinja_env.profile =
        CodeGenerationProfile::TypeCheck(function_signatures.clone(), resolved_context.clone());
    ...
    let source = content.to_string();
    let tmpl =
        match jinja_env.template_from_named_str(relative_file_path.to_str().unwrap(), &source) {
            Ok(tmpl) => tmpl,
            Err(e) => {
                emit_error_log_message(
                    ErrorCode::Generic,
    ...
    let result = if jinja_typechecking_listener_factory.requires_full_typecheck() {
        tmpl.typecheck(
            function_signatures,
            builtins,
            listener.clone(),
