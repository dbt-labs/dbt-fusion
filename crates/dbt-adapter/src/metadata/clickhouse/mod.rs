...
impl MetadataAdapter for ClickHouseMetadataAdapter {
    ...
    fn list_relations_schemas_inner(
        ...
        let map_f = move |conn: &'_ mut dyn Connection,
            ...
            let schema = build_schema_from_clickhouse_describe(batch, adapter.engine().type_ops())?;
            Ok(schema)
        };
...
pub fn list_relations(
    ...
    let table_databases = get_column_values::<StringArray>(&batch, "table_database")?;
...
fn build_schema_from_clickhouse_describe(
    ...
    let column_names = get_column_values::<StringArray>(&describe_result, "name")?;
    let data_types = get_column_values::<StringArray>(&describe_result, "type")?;
