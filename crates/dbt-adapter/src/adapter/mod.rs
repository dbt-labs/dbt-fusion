    ...
    pub fn call_method_impl(
        ...
        match name {
            ...
            "get_relation" => {
                ...
                // Use next_kwarg (not next_arg) for database so that a database=None kwarg
                // from ClickHouse 2-part relations is explicitly consumed — next_arg would
                // fail on None and leave the kwarg in the iterator for finish() to reject.
                let database = match iter.next_kwarg::<Option<&str>>("database")? {
                    Some(db) if !db.is_empty() => db,
                    _ => {
                        if self.adapter_type() == AdapterType::Databricks {
                            DEFAULT_DATABRICKS_DATABASE
                        } else {
                            ""
                        }
                    }
            ...
            "check_incremental_schema_changes" => {
                ...
                // Stub: return None; only reached when on_schema_change != 'ignore'
                Ok(Value::from(()))
            }
