...
mod tests {
    ...
    fn test_strip_clickhouse_wrappers_case_insensitive() {
        // ClickHouse type names are case-insensitive on input; the driver may
        // emit either case.
        assert_eq!(
            ColumnBuilder::strip_clickhouse_wrappers("nullable(String)"),
            "String"
    ...
    fn test_strip_clickhouse_wrappers_leaves_other_wrappers_alone() {
        // `Array`, `Tuple`, `Map`, `DateTime64`, etc. are not nullability wrappers
        // and must not be stripped — they're legitimate type constructors.
        assert_eq!(
    ...
    fn test_parse_decimal_precision_scale() {
        ...
        // No-space variant accepted.
        assert_eq!(
            ColumnBuilder::parse_decimal_precision_scale("Decimal(38,9)"),
            Some((38, 9))
        );
        // ClickHouse-specific `Decimal32/64/128/256(scale)` shorthands carry only
        // the scale, so they fall through to the Arrow-typed precision-scale
        // path — confirm we reject them here.
        assert_eq!(
